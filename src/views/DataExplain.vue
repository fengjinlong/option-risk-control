  ...
  <div class="workspace">
    ...
    <div class="explain-body">
      ...
      <div class="section">
        ...
        <p><strong>📊 数据案例：</strong>交易员违规开仓，导致 <span class="inline-key">total_net_delta</span> 飙升至 15.0（远超 1.84 硬红线）。ETH
          突发回调，价格下跌 525 USD：</p>
      ...
      <div class="section">
        ...
        <p><strong>📊 数据案例：</strong>账户重仓卖出临期平值 Put，导致 <span class="inline-key">total_net_gamma = -0.0025</span>（远超
          0.000884）。ETH 从 3500 USD 暴跌到 3000 USD：</p>
      ...
      <div class="section">
        ...
        <p><strong>📊 数据案例：</strong>账户过度卖出远期虚值期权收割权利金，导致 <span class="inline-key">total_net_vega = -350.00 USD</span>（超出
          154.25 承受上限）。市场突发暴跌恐慌，大盘 IV 在 1 小时内暴涨 30%：</p>
      ...
      <div class="section">
        ...
        <p><strong>📊 数据案例：</strong>日内疯狂买入海量超短期末日彩票期权，导致 <span class="inline-key">total_net_theta = -8,500.00
            USD</span>（超过 7,712.53 单日最大流血阈值）。即使接下来 24 小时大盘完全横盘：</p>
        <div class="formula">时间无情流逝，刚性蒸发 8,500.00 USD 权利金</div>
        <p>标的尚未启动，账户就因时间磨损在合规层面被迫执行死线斩仓。</p>
      </div>

      <!-- IM/MM 分隔标题 -->
      <div class="page-title">资金健康度指标（IM / MM）动态阈值应用说明书</div>
      ...
      <div class="section">
        ...
        <p><strong>📊 数据案例：</strong>交易员在中栏沙盒频繁组装多腿卖方策略，导致 <span class="inline-key">initial_margin_usd</span> 瞬间飙升至
          98,000 USD（IM 占用率达 98%）。此时尝试再次提交一腿 ETH 虚值 Call，**该增量单需要新增 3,000 USD 的 IM，总需求（101,000）超过 100,000
          底牌，后端直接遭遇交易所网关强平拒单（Order reject: Insufficient available balance），沙盒模拟开仓宣告技术性流产。**</p>
      </div>
      ...
      <div class="section">
        ...
        <p><strong>面临问题：</strong>触发强制平仓清算（Forced Liquidation）。一旦市场发生不利波动，MM 需求会由于期权 Gamma 的非线性暴涨而急剧膨胀。如果 MM
          需求追上保证金资产，意味着已经无力承担现存仓位的最小潜在亏损。</p>
        <p><strong>📊 数据案例：</strong>标的 ETH 发生黑天鹅级大瀑布，Gamma 暴胀，账户 <span class="inline-key">maintenance_margin_usd</span>
          在 5 分钟内从 15,000 USD 疯狂膨胀至 100,005 USD。**此时，最低生存铁底（100,005）无情超越账户保证金总资产（100,000），UTA
          账户所有清算托管引擎瞬间激活，系统开始无预警强制砍仓，直到 MM 被拉回资产线以内。**</p>
      </div>
      ...
      <div class="section">
        ...
        <div class="risk-levels">
          ...
          <div class="risk-item attention">
            ...
            <span class="risk-desc">利用率达 65% 时，前端进度条由黄转橙。警告交易员当前仓位已开始累积波动率暴露，中栏沙盒在模拟新开仓时，必须优先评估能降 MM
              的对冲单，限制纯裸卖（Short）单的提交。</span>
          </div>
...
.risk-item.safe .risk-range {
  color: #16a34a;
}

.risk-item.safe .risk-name {
  color: #15803d;
}
...
.risk-item.attention .risk-range {
  color: #d97706;
}

.risk-item.attention .risk-name {
  color: #b45309;
}
...
.risk-item.high-risk .risk-range {
  color: #dc2626;
}

.risk-item.high-risk .risk-name {
  color: #b91c1c;
}
...
.risk-item.critical .risk-range {
  color: #991b1b;
}

.risk-item.critical .risk-name {
  color: #7f1d1d;
}
</style>
